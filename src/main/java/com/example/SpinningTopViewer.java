...
public class SpinningTopViewer {
    ...
    private void loop() {
        ...
        glShadeModel(GL_SMOOTH);    // Smooth shading (deprecated in modern OpenGL)

        // Lighting is disabled for compatibility with modern OpenGL core profiles.
        // If you want lighting, implement it in shaders.
        ...
        while ( !glfwWindowShouldClose(window) ) {
            ...
            simulation.evolute(time, time + simulation.getDt());
     ...
     * @param upZ
     */
    // Modern OpenGL does not support matrix stack operations.
    // If you are using compatibility profile and have access to GLU, you can use gluLookAt.
    // Otherwise, you should implement camera/view matrix math and upload it to your shaders.
    private void lookAt(
            double eyeX, double eyeY, double eyeZ,
            double centerX, double centerY, double centerZ,
            double upX, double upY, double upZ) {
        // Custom implementation of lookAt, since GLU is not available.
        double[] f = {
            centerX - eyeX,
            centerY - eyeY,
            centerZ - eyeZ
        ...
        // Normalize f
        double rlf = 1.0 / Math.sqrt(f[0]*f[0] + f[1]*f[1] + f[2]*f[2]);
        f[0] *= rlf; f[1] *= rlf; f[2] *= rlf;

        // Normalize up
        double[] up = { upX, upY, upZ };
        double rlu = 1.0 / Math.sqrt(up[0]*up[0] + up[1]*up[1] + up[2]*up[2]);
        up[0] *= rlu; up[1] *= rlu; up[2] *= rlu;
        ...
        // Normalize s
        double rls = 1.0 / Math.sqrt(s[0]*s[0] + s[1]*s[1] + s[2]*s[2]);
        s[0] *= rls; s[1] *= rls; s[2] *= rls;
